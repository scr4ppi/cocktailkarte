...
const cocktailProfiles = [
  ...
  { id: "paloma", name: "Paloma", note: "Herb-fruchtig, frisch und tequila-basiert.", fruity: 4, alcohol: 3, sweet: 2, sour: 3, bitter: 3, creamy: 0 },
  ...
  { id: "blue-lagoon", name: "Blue Lagoon", note: "Süß-sauer, frisch und auffällig.", fruity: 4, alcohol: 3, sweet: 4, sour: 3, bitter: 0, creamy: 0 },
...
function getBerlinDateParts(date = new Date()) {
  ...
  // Wochentag absolut browser-sicher über ein natives Datumsobjekt ermitteln
  ...
  return {
    ...
    weekday: exactDate.getDay() // 0 = Sonntag, 1 = Montag, ..., 6 = Samstag
  };
}
...
function getRotationPlanForDate(dateObj) {
  ...
  const diffDays = Math.round(diffMs / 86400000); // Math.round verhindert Verschiebungen durch Sommer-/Winterzeit
...
// Prüft, ob eine prozentuale Rabatt-Aktion aktiv ist (Samstag von 20 bis 22 Uhr)
function getGlobalDiscountRate() {
  const now = getBerlinDateParts();
  
  if (now.weekday === 6) { // Samstag
    if (now.hour === 20) return 0.30; // 20:00 - 20:59 Uhr -> 30%
    if (now.hour === 21) return 0.10; // 21:00 - 21:59 Uhr -> 10%
  }
  return 0;
}
...
function areSpecialsVisible() {
  ...
  return (
    (now.weekday === 6 && now.hour >= 20) || // Sichtbar ab Samstag 20 Uhr (wegen der neuen Rabatte)
    (now.weekday === 0 && now.hour < 10)
  );
}

// Reguläre Specials greifen erst NACH den Rabattstunden (ab 22 Uhr)
...
function applyWeeklySpecials(cards) {
  ...
  // 1. Prüfen, ob eine prozentuale Rabattphase aktiv ist (20-21 Uhr oder 21-22 Uhr)
  ...
  if (globalDiscount > 0) {
    cards.forEach((card) => {
      ...
      if (!isNaN(basePrice)) {
        ...
        // Ändert den Text in z.B. "5,25€ (-30%)"
    ...
    // Wichtig: Wenn globaler Rabatt aktiv ist, überspringen wir die normalen Specials für diese Zeit!
    return;
  }

  // 2. Regulärer Rotationsplan (Greift ab 22:00 Uhr)
...
// Zeichnet das interaktive Geschmacks-Radar-Chart
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initiales Triggern beim Laden der Website
  ...
  // Das Intervall prüft jede Minute automatisch, ob eine neue Rabattstufe erreicht ist
  setInterval(() => {
    applyWeeklySpecials(cards);
    updateSpecialButtonVisibility();
    applyCurrentFilter(cards);
